      ...
      {tenantMode && (
        <>
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <div className="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2.5 text-zinc-950">
                  ...
                  <div>
                    <h3 className="text-base font-bold">Phần mềm chọn ảnh RAW</h3>
                    <p className="text-xs text-zinc-500">Đồng bộ và chọn ảnh trực tiếp từ máy tính</p>
                  </div>
                </div>

                <div className="mt-4 space-y-3.5">
                  <div className="rounded-md bg-zinc-50 p-3 text-xs text-zinc-600 border border-zinc-150">
                    <span className="font-bold block text-zinc-900 mb-0.5">Cách kích hoạt:</span>
                    Tải và cài đặt phần mềm trên máy tính, đăng nhập hoặc sử dụng mã License Key bên dưới để kích hoạt thiết bị.
                  </div>

                  {license ? (
                    <div className="space-y-2">
                      <div className="flex items-center justify-between gap-2 text-xs">
                        <span className="font-semibold text-zinc-500">License Key:</span>
                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold ${
                          license.status === "active" ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"
                        }`}>
                          {license.status === "active" ? "Đang hoạt động" : "Hết hạn/Khóa"}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 rounded bg-zinc-100 p-2 text-sm font-mono text-zinc-800">
                        <span className="flex-1 select-all tracking-wider font-semibold">{license.license_key}</span>
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="text-zinc-500 hover:text-zinc-950 transition p-1"
                          title="Sao chép Key"
                        >
                          <Copy size={16} />
                        </button>
                      ...
                      {license.expires_at && (
                        <p className="text-[11px] text-zinc-500">
                          Hạn dùng: {new Date(license.expires_at).toLocaleDateString("vi-VN")}
                        </p>
                      )}
                    </div>
                  ) : (
                    <p className="text-xs text-zinc-500 italic">
                      Chưa tạo license cho studio này. Bạn có thể kích hoạt bằng tài khoản chủ studio trên app.
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="mt-5">
                <a
                  href="/downloads/tlora-raw-selector-installer.exe"
                  className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-md bg-zinc-950 hover:bg-zinc-800 px-4 text-sm font-bold text-white shadow-sm transition-all"
                >
                  <Download size={15} /> Tải phần mềm quản lý (Windows)
                </a>
              </div>
            </div>
          </div>
