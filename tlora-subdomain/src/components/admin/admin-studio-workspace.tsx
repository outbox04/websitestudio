...
type AdminStudioWorkspaceProps = {
  ...
  license?: { license_key: string; status: string; expires_at: string | null } | null;
};
        ...
        <aside className="border-r border-[#2a2722] bg-[#07080a] px-4 py-5 text-[#f8f5ee] lg:sticky lg:top-0 lg:flex lg:min-h-screen lg:flex-col">
          ...
          <div className="mt-auto hidden border-t border-white/10 pt-4 lg:block">
            <div className="flex items-center gap-3 rounded-lg bg-white/[.04] p-3">
              ...
              <div className="min-w-0"><p className="truncate text-sm font-bold">{studioName || "TLORA Studio"}</p><p className="text-xs text-[#8c8174]">Quản trị viên</p></div>
            </div>
          </div>
        </aside>
          ...
          {activeView === "site-builder" && (
            <div className="space-y-5">
              <div className="flex flex-col justify-between gap-4 rounded-xl border border-[#2a2722] bg-[#101115] p-5 text-[#f8f5ee] sm:flex-row sm:items-center">
                <div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#d8b766]">Website CMS</p><h1 className="mt-2 text-2xl font-extrabold">Biên tập website trực quan</h1><p className="mt-1 text-sm text-[#cbc0b0]">Thay đổi nội dung, hình ảnh và nhận diện rồi xem kết quả ngay trên website thật.</p></div>
              ...
              {cmsPanel === "content" ? <StudioSiteBuilder mode="builder" studioSettings={studioSettings} studioSlug={studioSlug} /> : <StudioSiteBuilder mode="settings" studioSettings={studioSettings} studioSlug={studioSlug} />}
            </div>
          )}
    ...
    <div className="space-y-5">
      <section className="overflow-hidden rounded-xl border border-[#2a2722] bg-[#101115] text-[#f8f5ee]">
        <div className="flex flex-col justify-between gap-5 p-6 md:flex-row md:items-center">
          <div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#d8b766]">Website overview</p><h1 className="mt-2 text-3xl font-extrabold">Tổng quan website</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#cbc0b0]">Theo dõi mức độ sẵn sàng, nội dung, nhận diện và các công cụ đo lường của website studio.</p></div>
          <a href={publicUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-[#d8b766] px-5 text-sm font-bold text-[#07080a] transition hover:bg-[#f3d88e]">Mở website <ExternalLink size={16} /></a>
        </div>
        <div className="border-t border-white/10 bg-[#14110f] px-6 py-3 text-xs text-[#8c8174]">{studioSlug || "TLORA"} · {setupCompleted ? "Đang hoạt động" : "Chưa xuất bản"}</div>
      </section>
      ...
      <div className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
        <section className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
          <div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-extrabold">Sức khỏe website</h2><p className="mt-1 text-sm text-zinc-600">Các hạng mục cần thiết để website sẵn sàng tăng trưởng.</p></div><MonitorSmartphone className="text-zinc-500" size={22} /></div>
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            ...
            <HealthItem label="Gói website" ready={license?.status === "active"} detail={license?.status === "active" ? "Giấy phép đang hoạt động" : "Cần kiểm tra giấy phép dịch vụ"} />
          </div>
        </section>
        <section className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-[.14em] text-[#a57f2c]">Hành động đề xuất</p><h2 className="mt-2 text-xl font-extrabold">{setupCompleted ? "Tiếp tục tối ưu" : "Xuất bản website"}</h2>
...
function WebsiteMetric({ icon: Icon, label, value, helper, tone = "default" }: { icon: LucideIcon; label: string; value: string; helper: string; tone?: "default" | "success" | "warning" }) {
...
function HealthItem({ label, ready, detail }: { label: string; ready: boolean; detail: string }) {
  return (
    <div className="flex items-start gap-3 rounded-lg border border-zinc-200 p-4">
      <span className={`mt-0.5 grid size-6 shrink-0 place-items-center rounded-full ${ready ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>{ready ? <Check size={14} /> : <span className="size-1.5 rounded-full bg-current" />}</span>
      <div><p className="text-sm font-bold text-zinc-900">{label}</p><p className="mt-1 text-xs leading-5 text-zinc-500">{detail}</p></div>
    </div>
  );
}
