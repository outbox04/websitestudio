    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
        {albumPhotos.map((photo) => {
          ...
          return (
            <article key={photo.id} className="overflow-hidden rounded-lg border border-white/10 bg-white/[0.04] shadow-xl shadow-black/20">
              <div className="relative aspect-[4/5] bg-zinc-950">
                ...
                <button
                  ...
                  className="absolute right-3 top-3 grid size-10 place-items-center rounded-md bg-black/35 text-white shadow backdrop-blur"
                ...
                <button
                  ...
                  className={`absolute left-3 top-3 flex min-h-10 items-center gap-2 rounded-md px-3 text-sm font-semibold shadow ${
                    isSelected ? "bg-[#d8b766] text-black" : "bg-black/35 text-white backdrop-blur"
                  }`}
                >
                  {isSelected ? <Check size={18} /> : <span className="size-4 rounded border border-current" />}
              ...
              <div className="space-y-3 p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="rounded-md bg-white/10 px-2 py-1 text-xs font-medium text-zinc-300">{photo.status}</span>
                </div>
                <label className="block text-sm font-medium text-zinc-300" htmlFor={`note-${photo.id}`}>
                  Ghi chú chỉnh sửa
                </label>
                <textarea
                  id={`note-${photo.id}`}
                  ...
                  className="min-h-24 w-full resize-none rounded-md border border-white/10 bg-black/25 p-3 text-sm text-white outline-none transition placeholder:text-zinc-500 focus:border-[#d8b766]"
                />
              </div>
            </article>
          );
      ...
      <button
        ...
        className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-[#d8b766] px-5 py-3 text-sm font-semibold text-black"
      >
        {saving && <Loader2 className="animate-spin" size={16} />}
        Lưu lựa chọn ảnh
      </button>
      {preview && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/90 p-4" role="dialog" aria-modal="true">
          <div className="relative h-[86vh] w-full max-w-5xl overflow-hidden rounded-lg border border-white/10 bg-[#101115]">
            <Image src={preview.src} alt={preview.name} fill sizes="90vw" className="object-contain" />
            <button onClick={() => setPreview(null)} className="absolute right-3 top-3 grid size-10 place-items-center rounded-full bg-white/10 text-white shadow backdrop-blur" aria-label="Đóng">
              <X size={20} />
            </button>
          </div>
        </div>
