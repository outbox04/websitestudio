...
import { Check, Coins, ImageUp, Loader2, LogIn, Sparkles, UserPlus, Wallet } from "lucide-react";
    ...
    return (
      <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
        <section className="relative overflow-hidden rounded-lg border border-white/10 bg-white/[0.04] p-6 shadow-2xl shadow-black/25 md:p-10">
          ...
          <p className="mt-8 text-sm font-semibold uppercase tracking-[0.18em] text-[#d8b766]">AI Concept 50.000đ / ảnh</p>
          <h1 className="mt-3 max-w-3xl font-heading text-4xl font-extrabold leading-tight text-white md:text-6xl">
            Tạo ảnh ghép concept cao cấp trước khi đầu tư buổi chụp lớn.
          </h1>
          <p className="mt-5 max-w-2xl text-base leading-7 text-zinc-300">
            Đăng ký tài khoản, nạp tiền và tạo ảnh concept từ ảnh chân dung của bạn với preset trang phục, background và phong cách riêng.
          </p>
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {["Giá rõ ràng 50k/ảnh", "Giữ nhận diện khuôn mặt", "Lưu lịch sử concept"].map((item) => (
              <div key={item} className="flex items-center gap-2 rounded-md border border-white/10 bg-black/25 px-3 py-3 text-sm font-semibold text-zinc-200">
                <Check size={17} className="text-[#d8b766]" />
                {item}
              </div>
            ))}
          </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="rounded-lg border border-white/10 bg-white/[0.04] p-5 shadow-2xl shadow-black/25">
          <div className="flex flex-col justify-between gap-4 border-b border-white/10 pb-5 md:flex-row md:items-start">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#d8b766]">AI Concept Studio</p>
              <h1 className="mt-3 font-heading text-3xl font-extrabold text-white md:text-5xl">Tạo ảnh ghép concept</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-400">
                Mỗi lượt tạo ảnh trừ 50.000đ từ ví. API chỉ trừ tiền khi OpenAI trả ảnh thành công.
              </p>
            </div>
          ...
          <div className="mt-5 grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
            <div className="rounded-lg border border-dashed border-white/15 bg-black/25 p-5">
              <div className="grid min-h-80 place-items-center text-center">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-6 text-zinc-400">Nên dùng ảnh chính diện, đủ sáng, không che mặt để concept giữ nhận diện tốt.</p>
                  <input
                    type="file"
            ...
            <div className="space-y-5">
              ...
              <PresetPicker title="Background" options={aiPresets.backgrounds} value={background} onChange={setBackground} />
          ...
          {resultImage && (
            <div className="mt-6 rounded-lg border border-white/10 bg-black/30 p-4">
              <h2 className="text-lg font-bold text-white">Ảnh concept vừa tạo</h2>
              <div className="relative mt-4 aspect-[2/3] w-full max-w-sm overflow-hidden rounded-md border border-white/10 bg-black">
                <Image src={resultImage} alt="Ảnh concept AI" fill sizes="384px" className="object-cover" unoptimized />
              </div>
            </div>
          )}
        </section>
        ...
        <aside className="space-y-5">
          <section className="rounded-lg border border-white/10 bg-white/[0.04] p-5 shadow-2xl shadow-black/20">
            ...
            <p className="mt-3 text-sm text-zinc-400">Có thể tạo thêm {remainingTurns} ảnh concept.</p>
            ...
            <button onClick={topUp} disabled={topUpLoading} className="mt-3 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-md border border-white/10 bg-white/10 px-4 text-sm font-semibold text-white hover:bg-white/15">
          ...
          <section className="rounded-lg border border-white/10 bg-white/[0.04] p-5">
            <h2 className="text-lg font-bold text-white">Lịch sử gần đây</h2>
            <div className="mt-4 space-y-3">
              {history.length === 0 ? (
                <p className="text-sm leading-6 text-zinc-400">Chưa có ảnh concept nào được tạo.</p>
              ) : (
                history.map((item) => (
...
function AuthPanel({
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-lg border border-white/10 bg-white p-6 text-zinc-950 shadow-2xl shadow-black/30">
      ...
      <div className="mt-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600">Dùng tài khoản để quản lý ví và lịch sử ảnh.</p>
        </div>
      </div>
