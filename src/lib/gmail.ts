...
export async function sendActivationEmail(data: ActivationEmail) {
  ...
  const subject = `=?UTF-8?B?${Buffer.from("TLORA Studio — Kích hoạt thành công").toString("base64")}?=`;
  const raw = [`From: TLORA Studio <${sender}>`, `To: ${data.to}`, `Subject: ${subject}`, "MIME-Version: 1.0", `Content-Type: multipart/alternative; boundary=${boundary}`, "", `--${boundary}`, "Content-Type: text/plain; charset=UTF-8", "Content-Transfer-Encoding: base64", "", Buffer.from(text).toString("base64"), `--${boundary}`, "Content-Type: text/html; charset=UTF-8", "Content-Transfer-Encoding: base64", "", Buffer.from(html).toString("base64"), `--${boundary}--`].join("\r\n");
