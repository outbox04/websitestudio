...
function parseEditRequestRows(response: unknown): EditRequestRow[] {
  ...
  return source
    .map((item) => {
      if (typeof item === "string") {
        return { fileName: item.trim(), description: "" };
      }

      const fileName = readStringField(item, [
        "fileName",
        "filename",
        "name",
        "tenAnh",
      ...
      const description = readStringField(item, [
        "description",
        "editDescription",
        "edit_description",
        "moTa",
...
function rowsFromText(value: string, existingRows: EditRequestRow[]) {
  const descriptions = new Map(existingRows.map((row) => [normalizeName(row.fileName), row.description]));
  return value
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean)
    .map((fileName) => ({
      fileName,
      description: descriptions.get(normalizeName(fileName)) ?? "",
    }));
}
...
export function EditSyncPage() {
  ...
  const [editRequestRows, setEditRequestRows] = useState<EditRequestRow[]>([]);
  ...
  const visibleEditRows = useMemo(
    () => rowsFromText(selectedFilesText, editRequestRows),
    [selectedFilesText, editRequestRows],
  );
      ...
      <div className="page-shell space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-xl border border-border bg-navy-soft/30">
              <div className="grid grid-cols-[minmax(140px,240px)_minmax(220px,1fr)] gap-3 border-b border-border px-4 py-3 text-xs font-semibold uppercase text-ink-faint max-sm:hidden">
                <div>Tên ảnh</div>
                <div>Mô tả nội dung chỉnh sửa</div>
              </div>
              {visibleEditRows.length > 0 ? (
                <div className="divide-y divide-border/60">
                  {visibleEditRows.map((row, index) => (
                    <div
                      key={`${row.fileName}-${index}`}
                      className="grid gap-2 px-4 py-3 text-sm sm:grid-cols-[minmax(140px,240px)_minmax(220px,1fr)] sm:gap-3"
                    >
                      <div className="min-w-0 font-semibold text-ink">
                        <span className="mb-1 block text-[11px] uppercase text-ink-faint sm:hidden">Tên ảnh</span>
                        <span className="break-all">{row.fileName}</span>
                      </div>
                      <div className="min-w-0 text-ink-muted">
                        <span className="mb-1 block text-[11px] uppercase text-ink-faint sm:hidden">Mô tả nội dung chỉnh sửa</span>
                        <span className="whitespace-pre-wrap break-words">{row.description || "Chưa có mô tả"}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="px-4 py-6 text-center text-sm text-ink-muted">
                  Chưa có danh sách ảnh cần chỉnh.
                </div>
              )}
            </div>
