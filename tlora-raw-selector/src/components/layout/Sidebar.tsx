...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "glass-panel flex h-full flex-col shrink-0 transition-[width] duration-200 max-md:w-[76px]",
          ...
          >
            {({ isActive }) => (
              <>
                ...
                {!sidebarCollapsed && <span className="truncate max-md:hidden">{label}</span>}
              </>
            )}
          </NavLink>
        ))}
        ...
        >
          {sidebarCollapsed ? (
            ...
            <>
              ...
              <span className="max-md:hidden">Thu gọn</span>
            </>
          )}
        </button>
      </div>
