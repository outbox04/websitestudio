...
import { CheckCircle2, ChevronDown, Eye, EyeOff, FolderOpen, Loader2, LogIn, Save, XCircle } from "lucide-react";
...
export function SettingsPage() {
  ...
  useEffect(() => {
    ...
    void listen<GoogleDriveOAuthCallbackPayload>("google-drive-oauth-callback", (event) => {
      ...
      void exchangeGoogleCode(latestSettingsRef.current, payload.code, payload.state ?? null)
        .then(() => ensureTloraDriveFolder(latestSettingsRef.current))
        .then((folder) => {
          const next = {
          ...
          setDriveTest({ state: "ok", message: `Da ket noi Drive: ${folder.name}` });
        })
  ...
  async function handleTestDrive() {
    setDriveTest({ state: "testing", message: "" });
    try {
      let folderId = parseDriveFolderId(local.google_drive_root_folder_id);
      let folder: { id: string; name: string; mimeType: string };
      if (!folderId) {
        folder = await ensureTloraDriveFolder(local);
        folderId = folder.id;
      } else {
        const token = await getValidGoogleToken(local);
        folder = await checkDriveFolderWithToken(folderId, token.access_token);
      }
      if (folderId !== local.google_drive_root_folder_id) {
        const next = { ...local, google_drive_root_folder_id: folderId };
        setLocal(next);
        setDirty(false);
        await saveSettings(next);
      }
      await openUrl(`https://drive.google.com/drive/folders/${encodeURIComponent(folderId)}`);
      setDriveTest({ state: "ok", message: `Drive OK: ${folder.name}` });
    } catch (e) {
      setDriveTest({ state: "fail", message: e instanceof Error ? e.message : String(e) });
    }
  }
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3 pt-1">
              <Button variant="outline" size="sm" onClick={() => void handleGoogleLogin()} disabled={saving}>
              ...
              <Button
                ...
                disabled={driveTest.state === "testing"}
              >
                {driveTest.state === "testing" && <Loader2 className="h-3.5 w-3.5 animate-spin" />}
                Kiểm tra
              </Button>
