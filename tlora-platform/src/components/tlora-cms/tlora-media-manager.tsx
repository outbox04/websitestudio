...
export function TloraMediaManager({ initialMedia }: { initialMedia: TloraCmsMediaAsset[] }) {
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      setMessage(`Đã tối ưu ${(file.size / 1024).toFixed(0)} KB → ${(optimized.file.size / 1024).toFixed(0)} KB và thêm vào Media Library.`);
    } catch (error) {
  ...
  return (
    <main className="min-h-screen bg-[#f4f4f2] p-4 text-zinc-950 sm:p-6 lg:p-8">
      <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#a57f2c]">TLORA assets</p><h1 className="mt-2 text-3xl font-extrabold">Media Library</h1><p className="mt-2 text-sm text-zinc-600">Ảnh được tự động thu về tối đa 1920px, chuyển WebP và cân bằng chất lượng/dung lượng trước khi tải lên.</p></div><><input ref={input} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); event.currentTarget.value = ""; }} /><button type="button" onClick={() => input.current?.click()} disabled={Boolean(busy)} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-zinc-950 px-4 text-sm font-bold text-white disabled:opacity-50">{busy === "upload" ? <Loader2 className="animate-spin" size={17} /> : <ImagePlus size={17} />} Tải ảnh lên</button></></header>
