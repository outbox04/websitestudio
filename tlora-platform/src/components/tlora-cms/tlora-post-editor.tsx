  ...
  return (
    <main className="min-h-screen bg-[#f4f4f2] p-4 text-zinc-950 sm:p-6 lg:p-8">
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <section>
          <input value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value, slug: current.id ? current.slug : slugify(event.target.value) }))} placeholder="Nhập tiêu đề bài viết" className="w-full border-0 bg-transparent text-4xl font-black outline-none" />
          <label className="mt-4 block text-sm font-bold">Đường dẫn<input value={form.slug} onChange={(event) => setForm((current) => ({ ...current, slug: slugify(event.target.value) }))} className="mt-2 min-h-11 w-full rounded-md border border-zinc-300 bg-white px-3 font-normal" /></label>
          <div className="mt-5 overflow-hidden rounded-xl border border-zinc-300 bg-white">
            <div className="flex border-b border-zinc-200"><button onClick={() => setMode("visual")} className={`px-4 py-3 text-sm font-bold ${mode === "visual" ? "bg-zinc-950 text-white" : ""}`}>Soạn thảo</button><button onClick={() => setMode("html")} className={`px-4 py-3 text-sm font-bold ${mode === "html" ? "bg-zinc-950 text-white" : ""}`}>HTML</button></div>
            {mode === "visual" ? <><div className="sticky top-0 z-10 flex flex-wrap items-center gap-1 border-b border-zinc-200 bg-zinc-50 p-2"><select aria-label="Cỡ chữ" onMouseDown={rememberSelection} onChange={(event) => command("fontSize", event.target.value)} className="h-9 rounded border px-2 text-sm"><option value="3">16px</option><option value="4">18px</option><option value="5">24px</option><option value="6">32px</option></select><Tool label="H1" icon={<Heading1 />} onClick={() => command("formatBlock", "h1")} /><Tool label="H2" icon={<Heading2 />} onClick={() => command("formatBlock", "h2")} /><button onMouseDown={rememberSelection} onClick={() => command("formatBlock", "h3")} className="h-9 rounded border px-2 text-sm font-bold">H3</button><button onMouseDown={rememberSelection} onClick={() => command("formatBlock", "p")} className="h-9 rounded border px-2 text-sm">Đoạn văn</button><Tool label="Đậm" icon={<Bold />} onClick={() => command("bold")} /><Tool label="Nghiêng" icon={<Italic />} onClick={() => command("italic")} /><Tool label="Gạch chân" icon={<Underline />} onClick={() => command("underline")} /><Tool label="Liên kết" icon={<Link2 />} onClick={openLink} /><Tool label="Thêm ảnh" icon={<ImagePlus />} onClick={() => { rememberSelection(); editingImageRef.current = null; setImageDialogInitial({ width: "100%" }); setImageOpen("content"); }} /></div>{linkOpen && <div className="flex gap-2 border-b border-zinc-200 bg-amber-50 p-2"><input autoFocus value={linkUrl} onChange={(event) => setLinkUrl(event.target.value)} placeholder="https://..." className="min-h-9 flex-1 rounded border px-3 text-sm" /><button onClick={applyLink} className="grid size-9 place-items-center rounded bg-zinc-950 text-white"><Check size={16} /></button><button onClick={() => setLinkOpen(false)} className="grid size-9 place-items-center"><X size={16} /></button></div>}<div ref={editorRef} contentEditable suppressContentEditableWarning onClick={(event) => { if ((event.target as HTMLElement).closest("a")) event.preventDefault(); }} onInput={(event) => { setForm((current) => ({ ...current, body: event.currentTarget.innerHTML })); rememberSelection(); }} onMouseUp={rememberSelection} onKeyUp={rememberSelection} onDoubleClick={(event) => { const target = event.target; if (target instanceof HTMLImageElement) { editingImageRef.current = target; setImageDialogInitial({ asset: media.find((asset) => asset.publicUrl === target.getAttribute("src")), width: target.style.width || "100%" }); setImageOpen("content"); } }} className="prose prose-zinc min-h-[520px] max-w-none p-6 text-base leading-8 outline-none" /></> : <textarea value={form.body} onChange={(event) => setForm((current) => ({ ...current, body: event.target.value }))} className="min-h-[620px] w-full bg-zinc-950 p-5 font-mono text-sm leading-7 text-emerald-200 outline-none" />}
          </div>
        </section>
        <aside className="space-y-5">
          <Panel title="Ảnh đại diện / OG Meta"><button type="button" onClick={() => { setImageDialogInitial({ asset: media.find((asset) => asset.publicUrl === form.coverImageUrl), width: "100%" }); setImageOpen("cover"); }} className="aspect-video w-full rounded-md border border-dashed border-zinc-300 bg-zinc-100 bg-cover bg-center" style={form.coverImageUrl ? { backgroundImage: `url(${form.coverImageUrl})` } : undefined}>{!form.coverImageUrl && "Chọn ảnh đại diện"}</button></Panel>
          ...
          {!valid && <p className="text-xs font-semibold text-red-600">Cần nhập tiêu đề, slug, mô tả SEO, nội dung và ảnh đại diện.</p>}
        </aside>
      </div>
...
function Tool({ label, icon, onClick }: { label: string; icon: React.ReactNode; onClick: () => void }) { return <button type="button" title={label} onMouseDown={(event) => event.preventDefault()} onClick={onClick} className="grid size-9 place-items-center rounded border bg-white [&_svg]:size-16">{icon}</button>; }
