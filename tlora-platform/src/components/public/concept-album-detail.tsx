...
import { ArrowLeft, Check, ChevronLeft, ChevronRight, Images, Loader2, Minus, Plus, ReceiptText, X } from "lucide-react";
...
function albumImages(album: TloraConceptAlbum) {
  return Array.from(new Set(album.images.filter((source) => Boolean(source) && source !== album.coverImageUrl)));
}
...
export function ConceptAlbumDetail({ album, similarAlbums, categoryHighlights }: DetailProps) {
  ...
  useEffect(() => {
    function keyboard(event: KeyboardEvent) {
      if (quoteOpen) return;
      if (images.length && event.key === "ArrowLeft") setActive((current) => (current - 1 + images.length) % images.length);
      if (images.length && event.key === "ArrowRight") setActive((current) => (current + 1) % images.length);
  ...
  return (
    <main className="min-h-screen bg-[#07080a] text-[#f8f5ee]">
      <section className="mx-auto max-w-7xl px-4 pb-12 pt-8 sm:px-6 lg:px-8 lg:pt-12">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1.45fr)_minmax(310px,.55fr)] lg:items-center">
          <div className="min-w-0">
            <div className="relative isolate h-[min(70vw,620px)] min-h-[360px] overflow-hidden rounded-xl bg-[#101115]" aria-label={`Carousel 360 độ của ${album.title}`}>
              {images.map((source, index) => {
                ...
                return <button key={`${source}-${index}`} type="button" onClick={() => offset === 0 ? setLightbox(true) : setActive(index)} tabIndex={offset === 0 ? 0 : -1} aria-label={offset === 0 ? `Xem ảnh ${index + 1}` : `Chuyển đến ảnh ${index + 1}`} className="absolute inset-y-[5%] left-1/2 w-[78%] transition-[transform,opacity] duration-500 ease-out focus-visible:outline-2 focus-visible:outline-[#d8b766]" style={{ opacity: visible ? offset === 0 ? 1 : 0.42 : 0, pointerEvents: visible ? "auto" : "none", zIndex: 10 - Math.abs(offset), transform: `translateX(calc(-50% + ${offset * 22}%)) perspective(1200px) rotateY(${offset * -24}deg) scale(${offset === 0 ? 1 : 0.82})` }}><Image src={source} alt={`${album.title} — ảnh ${index + 1}`} fill priority={index === 0} quality={75} sizes="(min-width: 1024px) 62vw, 90vw" className="object-contain" /></button>;
              })}
              {images.length > 0 ? <><button type="button" onClick={() => move(-1)} aria-label="Ảnh trước" className="absolute left-3 top-1/2 z-20 grid size-11 -translate-y-1/2 place-items-center rounded-full border border-white/15 bg-black/65 backdrop-blur-md hover:border-[#d8b766]"><ChevronLeft /></button><button type="button" onClick={() => move(1)} aria-label="Ảnh sau" className="absolute right-3 top-1/2 z-20 grid size-11 -translate-y-1/2 place-items-center rounded-full border border-white/15 bg-black/65 backdrop-blur-md hover:border-[#d8b766]"><ChevronRight /></button><p className="absolute bottom-4 left-1/2 z-20 -translate-x-1/2 rounded-full bg-black/70 px-4 py-2 text-xs font-bold backdrop-blur-md">{active + 1} / {images.length}</p></> : <p className="absolute inset-0 grid place-items-center px-6 text-center text-sm text-[#8c8174]">Album đang cập nhật ảnh nội dung.</p>}
            </div>
      ...
      {lightbox && images.length > 0 && <Lightbox images={images} title={album.title} active={active} setActive={setActive} onClose={() => setLightbox(false)} />}
