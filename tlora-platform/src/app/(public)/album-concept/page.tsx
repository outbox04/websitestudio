...
export default async function ConceptAlbumsPage({ searchParams }: { searchParams: Promise<{ album?: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#07080a] px-4 py-16 text-[#f8f5ee] sm:px-6 lg:px-8">
      ...
      <div className="mx-auto max-w-6xl">{albums.length ? <ConceptAlbumsView albums={albums} categories={categories} initialSlug={album} /> : <p className="rounded-xl border border-dashed border-white/15 p-10 text-center text-[#8c8174]">Album Concept đang được cập nhật.</p>}</div>
    </main>
  );
}
