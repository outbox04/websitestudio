...
create unique index if not exists studio_payment_orders_basic_domain_unique_idx on public.studio_payment_orders(lower(domain)) where domain is not null and plan = 'basic' and status in ('pending', 'paid');
